# リリースチェックリスト

- 文書状態: 確定
- 最終更新日: 2026-09-04
- 親仕様: [リリース・配布運用仕様](README.md)

本書は各リリースで再利用する公開ゲートの基準である。初回リリース候補`1.0.0 (2)`の実施結果、証跡および保留事項は[初回リリース進行記録](initial-release-status.md)へ記録し、本書のチェックボックスを外部状態だけで一括完了にしない。

## 1. リリース準備

- [ ] `main`がクリーンで、対象コミットと担当者が確定している。
- [ ] versionNameとversionCodeが正しく、既アップロード値と重複しない。
- [ ] 変更内容、DBスキーマ、バックアップ形式、権限、SDK、データ収集の差分を確認した。
- [ ] 依存関係、SDK Index、Google Playポリシー期限、Target API要件を確認した。
- [ ] 日本語リリースノートとストア掲載差分を用意した。
- [ ] Google Play Console申請シートの全項目を現行Consoleと照合し、未入力カードと公開ブロッカーがない。
- [ ] ストア掲載文を確定文案から転記し、文字数、改行、広告表示および実装との一致を確認した。
- [ ] 512pxアイコン、フィーチャーグラフィック、スマートフォン・タブレット画像と代替テキストが画像仕様を満たす。
- [ ] プライバシーポリシー、利用規約、外部送信に関する公表およびapp-ads.txtをHTTPSで閲覧できる。
- [ ] 法的文書の制定日と改定日が実際の公開日程に一致し、必要な専門家確認を完了した。
- [ ] GitHub Pagesのカスタムドメイン、DNSおよびHTTPSが有効で、法的文書を認証・地域制限・JavaScriptなしでモバイルから全文閲覧できる。
- [ ] 本リポジトリの`main`へ法的サイト変更をマージした後、その`legal-site`を公開リポジトリへコピーし、正本側commitと公開側commitを記録した。
- [ ] GitHub Pagesのデプロイ完了後、変更したURLとアプリ設定画面からのプライバシーポリシー・利用規約遷移で最新版を確認した。
- [ ] 3文書の提供者名、連絡先、外部サービスおよび送信情報が実装、Google Play Console、UMPと一致する。
- [ ] Data safety、広告、対象年齢、コンテンツレーティング、権限申告が実装と一致する。
- [ ] Financial features、Health apps、Government apps、Advertising ID、アカウント作成有無を申請シートどおり申告した。
- [ ] アプリ内商品が未登録で、アプリとストア掲載に購入または広告削除の導線がない。

## 2. 自動ゲート

- [ ] `testDebugUnitTest`とRelease対象の単体試験が成功した。
- [ ] `lintDebug`と`lintRelease`が成功した。
- [ ] Debug APKとRelease AABをビルドできた。
- [ ] Room migrationとバックアップ互換性試験が全対応元バージョンから成功した。
- [ ] Compose UI、アクセシビリティ、適応レイアウトの自動試験が成功した。
- [ ] Pull Requestまたは手動CIの`connectedDebugAndroidTest`が成功し、Room Migration、RepositoryおよびCompose UIのinstrumented testをAPI 30で実行した。
- [ ] 通知、ウィジェット、祝日、広告、同意のFake結合試験が成功した。
- [ ] セキュリティ、Secret、CycloneDX SBOM、依存関係、Manifest検査が成功した。
- [ ] `node fastlane/verify-play-store.mjs --release`が成功し、掲載文と全画像が公開仕様を満たした。
- [ ] Macrobenchmarkと主要性能回帰が公開基準を満たした。
- [ ] ログ検査でユーザー入力、トークン、URI、広告情報が検出されなかった。
- [ ] `node tools/release/verify-readiness.mjs --artifacts`が成功し、Release成果物と同じActions artifactへ検査結果JSONを保存した。

## 3. Release成果物検査

- [ ] Application ID、表示名、versionName、versionCodeがRelease値である。
- [ ] Debuggable無効、R8・resource shrink有効、Baseline Profile適用済みである。
- [ ] 本番広告IDと法的URLが正しく、テスト値と開発URLがない。
- [ ] 最終Manifestの権限、exported、backup、cleartext設定が仕様どおりである。
- [ ] AABがUpload Keyで署名され、SHA-256を記録した。
- [ ] AABの署名者が1件だけで、証明書SHA-256がGoogle Playへ登録するUpload Keyの値と一致した。
- [ ] mapping、CycloneDX SBOM、ライセンス、Manifest、テスト結果をversionCodeへ紐付けた。
- [ ] `node tools/release/verify-readiness.mjs --release`が成功し、公開候補のcommit、法的文書、ストア画像、署名済みAABおよび各ハッシュが一致した。

## 4. 実機・Google Play検査

- [ ] API 26、33、36および最新Androidで主要フローを完了した。
- [ ] スマートフォン、タブレット、折りたたみ、ChromeOS、分割画面で確認した。
- [ ] 新規インストール、前公開版からの更新、プロセス再生成、再起動を確認した。
- [ ] TODO登録・編集・完了、履歴、アーカイブ、設定、バックアップ・復元を確認した。
- [ ] 通知権限、正確なアラーム、ロック画面通知、通知操作を確認した。
- [ ] ウィジェット追加、更新、完了、復元を確認した。
- [ ] Release AABにGoogle Play Billing SDKと`com.android.vending.BILLING`権限が含まれない。
- [ ] テスト広告とUMP地域デバッグを確認した。
- [ ] Internal testingのPre-launch reportに未解決の重大問題がない。

## 5. 公開承認

- [ ] P0とP1の動作確認項目がすべて合格した。
- [ ] 未解決P2の影響、回避策、対応時期、承認者を記録した。
- [ ] S0・S1の未解決障害がない。
- [ ] データ消失、履歴改変、通知誤発火または同意違反がない。
- [ ] 公開トラック、対象国、割合、監視担当、停止判断者を決めた。
- [ ] 初回公開または段階公開の制約を確認した。

## 6. 公開後

- [ ] Google Playの処理、審査、公開状態を確認した。
- [ ] アプリをGoogle Playから新規インストール・更新できることを確認した。
- [ ] 広告、app-ads.txtおよび3つの法的リンクを本番で確認した。
- [ ] Android vitals、Pre-launch report、レビュー、ポリシー警告を確認した。
- [ ] 段階公開は各段階を24時間以上監視し、拡大・停止判断を記録した。
- [ ] 100%公開後にGitタグ、公開日、最終AAB SHA-256、リリースノートを確定した。

## 7. 定期保守

- 毎週、依存関係更新とAndroid vitalsを確認する。
- 毎月、Google Playポリシー期限、SDKサポート期限、Target API、広告、UMPを確認する。
- 四半期ごとに3つの法的文書、Data safety、権限、通信先、依存SDK一覧を棚卸しする。
- Androidの新しい安定版公開時に互換性と対象SDK更新計画を確認する。
- 署名鍵、CI Secret、Play Console権限を半年ごとに棚卸しする。
- プロダクト終了時は新規インストール停止、既存ユーザーのデータ・法的文書・サポート期間を別途計画する。
