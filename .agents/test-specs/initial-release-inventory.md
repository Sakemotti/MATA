...
2026年9月16日時点では、項目単位の証跡が揃ったP0を245件、P1を151件`合格`、P0/P1の残り9件を`未実施`として登録した。P2を含む全423件では399件が合格、24件が未実施である。今回はカテゴリ行の削除メニュー、通常・アーカイブTODO件数を含む削除確認、選択中カテゴリ削除後のカテゴリ未設定への切替を追加合格とした。
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト327件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
