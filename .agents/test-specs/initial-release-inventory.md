...
2026年9月13日時点では、項目単位の証跡が揃ったP0を207件、P1を87件`合格`、P0/P1の残り111件を`未実施`として登録した。P2を含む全423件では297件が合格、126件が未実施である。今回は曜日プリセットの個別変更と、カテゴリ管理のカスタム並べ替え、削除完了、下書き破棄、プレビューの5件を追加合格とした。

## 3. 自動検査の証跡

| 対象 | 結果 | 証跡 | 対応する主な項目 |
| --- | --- | --- | --- |
| 単体テスト | 41 test suite、200件、失敗0・エラー0・スキップ0 | 2026-09-13のローカル`testDebugUnitTest`結果、[PR #204 CI run 34758657480](https://github.com/Sakemotti/MATA/actions/runs/34758657480) | `REL-001`、論理日・繰り返し・通知・ViewModel等の一部 |
...
| Instrumented test | API 30 x86_64で現行186件完了、失敗なし。API 34 x86_64でも142件時点で失敗なし | [PR #204 CI run 34758657480](https://github.com/Sakemotti/MATA/actions/runs/34758657480)、2026-09-12のローカル`:app:connectedDebugAndroidTest`結果 | `REL-027`、Room Migration・Repository・通知・設定変更・バックアップ・Compose UIの一部 |
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト225件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
