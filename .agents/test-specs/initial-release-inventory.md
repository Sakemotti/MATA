...
リリース必須のP0/P1は405件である。各項目書の`結果`列は再利用可能な原本として全件`未実施`のまま維持し、リリース候補ごとの実績は[項目別結果TSV](initial-release-results.tsv)へ記録する。2026年9月13日時点では、項目単位の証跡が揃ったP0を198件、P1を72件`合格`、P0/P1の残り135件を`未実施`として登録した。P2を含む全423件では271件が合格、152件が未実施である。versionCode 3向け17件、広告読込失敗時の`WGT-014`、環境別7セッション、制御異常系17件、TODO一覧の中核表示5件、TODO一覧の状態・操作8件、カレンダー中核計算5件、カレンダー履歴リポジトリ4件、カレンダー履歴の基本操作・表示8件、カテゴリ別TODO一覧8件、アーカイブ一覧・履歴照会6件、アーカイブ画面UI8件、TODO編集の入力・繰り返し8件、TODO編集の通知・プレビュー8件、カテゴリ管理の並び替え・名称検証・履歴整合性6件、および正式採用したカレンダー履歴のデイリープランナーUIは合格した。
...
| Instrumented test | API 30 x86_64で現行163件完了、失敗なし。API 34 x86_64でも142件時点で失敗なし | [PR #200 CI run 34748557262](https://github.com/Sakemotti/MATA/actions/runs/34748557262)、2026-09-12のローカル`:app:connectedDebugAndroidTest`結果 | `REL-027`、Room Migration・Repository・通知・設定変更・バックアップ・Compose UIの一部 |
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト199件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
