...
- 文書状態: 初版
- 最終更新日: 2026-08-10
- 対象仕様: [アプリ全体仕様](../app-spec.md)、[開発ガイドライン](../development-guidelines.md)、[全画面仕様](../screen-specs/README.md)、[収益化仕様](../monetization-specs/README.md)、[アプリ起動・初期化・復帰仕様](../startup-specs/README.md)、[MATAデザインシステム仕様](../design-system-specs/README.md)、[画面サイズ・適応レイアウト仕様](../adaptive-layout-specs/README.md)、[エラー処理・障害回復仕様](../error-handling-specs/README.md)、[性能・省電力仕様](../performance-specs/README.md)、[セキュリティ・プライバシー仕様](../security-privacy-specs/README.md)、[ログ・診断・品質監視仕様](../observability-specs/README.md)

## 1. 目的

MATAの実装が、現時点で確定しているアプリ全体仕様と6画面の仕様を満たすことを、リリース単位で再現可能な形で確認する。

画面仕様の受け入れ条件213件を1件ずつテストIDへ対応付ける。加えて、画面仕様本文の詳細確認37件と、画面単体の受け入れ条件だけでは確認しにくい論理日、繰り返し、祝日、通知、ウィジェット、端末内保存、プライバシー、ビルド品質の横断確認113件を定義する。実行結果を記録する項目は合計363件とする。
...
| [todo-editor.md](todo-editor.md) | SCR-002 TODO登録・編集 | `TE-001`〜`TE-030` |
...
| 5. 繰り返し | `RPT-001`〜`RPT-027`、`TE-008`〜`TE-014` |
...
| 17. 公開準備までに決定する項目 | 確定後に関係項目の合否条件を具体化し、未確定中は理由付きで`保留`とする |
...
| ID | 名前 | 一日の終了時刻 | 用途 |
| --- | --- | --- | --- |
| `CAT-NONE` | カテゴリ未設定 | 共通設定を使用 | 固定区分 |
| `CAT-000` | 日付境界0時 | 0:00 | カレンダー日と同じ論理日 |
| `CAT-004` | 日付境界4時 | 4:00 | 深夜跨ぎ |
| `CAT-023` | 日付境界23時 | 23:00 | 境界直前・直後 |
| `CAT-A` | 日常 | 0:00 | 並び順・色・アイコン確認 |
| `CAT-B` | ゲーム | 4:00 | 並び順・色・アイコン確認 |

### 5.2 TODO

最低限、次のTODOを用意する。

- 単発、毎日、平日のみ、曜日指定、毎月の指定日、月末、一定日数ごと、週X回、月X回を各1件以上
...
| `E2E-002` | 異なる終了時刻のカテゴリで深夜帯を表示 | 同じ瞬間に異なる対象論理日のTODOが正しく同居する |
