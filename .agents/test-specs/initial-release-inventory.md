...
2026年9月15日時点では、項目単位の証跡が揃ったP0を237件、P1を127件`合格`、P0/P1の残り41件を`未実施`として登録した。P2を含む全423件では367件が合格、56件が未実施である。今回はTODO編集の画面モード、初期値、カテゴリ順、日付UI切替、方式別一時値、回数期間境界、保存可否、保存後遷移、未保存確認の9件を追加合格とした。
...
| TODO編集の基本画面・保存境界 | 新規・編集モード、初期値と操作メニュー、カテゴリ順、単発・繰り返しの日付UI、方式別一時値、全週開始曜日と部分期間、実変更だけの保存可否、保存結果、未保存確認を専用unit/Compose UI test 9件で検証 | 2026年9月15日のローカル`testDebugUnitTest`とAPI 34 x86_64`:app:connectedDebugAndroidTest`成功 | `TE-001`、`TE-002`、`TE-004`、`TE-008`、`TE-010`、`TE-014`、`TE-024`、`TE-026`、`TE-027` |
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト295件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
