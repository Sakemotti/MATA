...
2026年9月14日時点では、項目単位の証跡が揃ったP0を216件、P1を101件`合格`、P0/P1の残り88件を`未実施`として登録した。P2を含む全423件では320件が合格、103件が未実施である。今回はTODO一覧の日付移動、カテゴリ見出し、長文・全状態行、期限超過表示の4件を追加合格とした。
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト248件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
