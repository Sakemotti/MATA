...
2026年9月16日時点では、P0 248件とP1 157件の全405件を`合格`として登録した。P2を含む全423件では408件が合格、任意優先度のP2 15件が未実施であり、不合格・保留・対象外は0件である。`REL-010`では、自身を除く全P0/P1の合格、未解決Issue 0件およびP2不合格 0件を集計し、総合動作確認の試験ゲートを合格とした。
...
| カテゴリ管理の並び替え・名称・履歴整合性 | ドラッグ中の一覧端自動スクロール、ドロップ保存と再表示、一覧・タブへの順序反映、名称の境界値・正規化・表記保持、編集・削除後の現在参照と確定済み履歴スナップショットを専用テスト7件で検証 | 2026年9月16日のローカル`testDebugUnitTest`とAPI 34 x86_64`:app:connectedDebugAndroidTest`成功 | `CM-007`、`CM-008`、`CM-010`、`CM-012`、`CM-013`、`CM-021`、`CM-023` |
...
| 設定画面の通知・正確なアラーム権限 | 通知権限の許可・拒否表示とAndroid通知設定、正確なアラームの対象判定・許可状態・フォールバック説明とシステム設定、各復帰時の状態再取得と通知再構成を専用Compose UI test 2件で検証 | 2026年9月16日のローカルAPI 34 x86_64`:app:connectedDebugAndroidTest`成功 | `ST-014`、`ST-015` |
| 設定画面の最大フォント・コンパクト表示 | フォント倍率200%・320×640dpで全設定行の内容・説明・現在値をスクロール表示し、スイッチ、バックアップ確認、復元導線、必要時の広告プライバシー設定が欠けず操作可能なことを専用Compose UI test 1件で検証 | 2026年9月16日のローカルAPI 34 x86_64`:app:connectedDebugAndroidTest`成功 | `ST-045` |
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト335件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
...
| 状態 | 試験ID | 根拠または残作業 |
| --- | --- | --- |
| 証跡あり | `REL-001`〜`REL-027` | 第3節の自動検査、全P0/P1集計、公開候補、静的レビュー、外部送信・法的サイト同期照合および署名証明書照合 |
| 一部確認 | なし | - |
| 未完了 | なし | - |

`REL-010`は2026年9月16日に最後のP0/P1として合格した。本結果は総合動作確認の試験ゲートであり、Closed testing期間、Production accessおよびPlay Console最終確認が完了するまで本番公開を許可するものではない。

## 6. Closed testing中の確認順

P0/P1の正式試験は完了した。次の手順はClosed testingと本番公開に関する外部ゲートの記録として維持し、[RELEASE_OWNER実機試験実施計画](release-owner-device-test-plan.md)と[残実機・環境試験計画](release-v3-device-verification-plan.md)は再試験が必要になった場合に使用する。
...
8. 全P0/P1と`REL-010`は合格済み。本番公開判定は外部ゲート完了後に更新する。
