...
| --- | --- | --- | --- | --- |
| Google Mobile Ads（AdMob）/ Google | IPアドレス、おおよその位置、広告ID・アプリセットID等の識別子、アプリ・広告操作、診断、端末・アプリ情報 | バナー広告の配信、表示、測定、不正防止 | 広告配信、パーソナライズ、測定、分析、不正防止、サービス改善 | 対象地域ではUMPのプライバシー設定と端末の広告設定を利用できる。広告削除権利が有効な間はMobile Ads SDKを初期化しない。 |
| User Messaging Platform（UMP）/ Google | IPアドレス、端末・アプリ情報、地域判定、同意・プライバシー選択状態、通信情報 | 広告に関する同意・選択肢の取得と管理 | 同意状態の管理、法令・ポリシー対応 | 対象地域ではアプリのプライバシー設定から選択肢を再表示できる。広告削除中も同意状態確認の通信が発生する場合がある。 |
| Google Play Billing / Google Playの契約主体となるGoogle関連会社 | 商品ID、価格照会、購入状態、購入トークン、端末・アプリ情報その他決済・権利確認に必要な情報 | 価格表示、購入、承認、復元、払い戻し・取消の反映 | 決済、購入管理、不正防止、法令・規約対応 | 購入操作は任意。価格照会と既存購入の復元にはGoogle Playとの通信が必要。 |
