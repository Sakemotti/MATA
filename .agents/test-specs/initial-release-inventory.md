...
2026年9月16日時点では、項目単位の証跡が揃ったP0を247件、P1を156件`合格`、P0/P1の残り2件を`未実施`として登録した。P2を含む全423件では406件が合格、17件が未実施である。今回は正確なアラームの対象外非表示、対象端末での許可状態と設定導線、設定画面へ戻った際の状態再取得と通知再構成を追加合格とした。
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト334件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
