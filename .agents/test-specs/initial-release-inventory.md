...
2026年9月15日時点では、項目単位の証跡が揃ったP0を238件、P1を130件`合格`、P0/P1の残り37件を`未実施`として登録した。P2を含む全423件では371件が合格、52件が未実施である。今回はTODO編集の次回予定と回数指定型の対象期間・必要・完了・残り回数プレビューを追加合格とした。
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト299件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
