...
2026年9月14日時点では、項目単位の証跡が揃ったP0を216件、P1を97件`合格`、P0/P1の残り92件を`未実施`として登録した。P2を含む全423件では316件が合格、107件が未実施である。今回はTODO一覧の通常表示と現在地ドロワー、全カテゴリ共通の論理日、週・月回数指定TODOの同日二重完了拒否、過去履歴詳細、定義済み更新契機での再評価の5件を追加合格とした。
...
| Instrumented test | API 30 x86_64で現行203件完了、失敗なし。API 34 x86_64でも142件時点で失敗なし | [PR #212 CI run 34799349988](https://github.com/Sakemotti/MATA/actions/runs/34799349988)、2026-09-12のローカル`:app:connectedDebugAndroidTest`結果 | `REL-027`、Room Migration・Repository・通知・設定変更・バックアップ・Compose UIの一部 |
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト244件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
