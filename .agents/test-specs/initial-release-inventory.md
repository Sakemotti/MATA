...
リリース必須のP0/P1は405件である。各項目書の`結果`列は再利用可能な原本として全件`未実施`のまま維持し、リリース候補ごとの実績は[項目別結果TSV](initial-release-results.tsv)へ記録する。2026年9月12日時点では、項目単位の証跡が揃ったP0を172件、P1を60件`合格`、P0/P1の残り173件を`未実施`として登録した。P2を含む全423件では233件が合格、190件が未実施である。versionCode 3向け17件、広告読込失敗時の`WGT-014`、環境別7セッション、制御異常系17件、TODO一覧の中核表示5件、カレンダー中核計算5件、カレンダー履歴リポジトリ4件、カテゴリ別TODO一覧8件、アーカイブ一覧・履歴照会6件、アーカイブ画面UI8件、および正式採用したカレンダー履歴のデイリープランナーUIは合格した。
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト161件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
