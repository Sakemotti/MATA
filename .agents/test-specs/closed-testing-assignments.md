...
| `DEV_AUTO` | `DEV` | 117 | UNIT、Repository、DB、計算、通知スケジューラ等の技術検査 |
...
`DEV_AUTO`の専用テスト117件は[自動試験証跡TSV](automated-test-evidence.tsv)でJUnitテストと1対1に対応する。既存テストが包括的に成功しただけで、未対応の試験IDを合格にしない。
