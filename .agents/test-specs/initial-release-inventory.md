...
リリース必須のP0/P1は387件である。各項目書の`結果`列は再利用可能な原本として全件`未実施`のまま維持し、リリース候補ごとの実績は[項目別結果TSV](initial-release-results.tsv)へ記録する。2026年9月4日時点では、項目単位の証跡が揃ったP0のリリース検査11件を`合格`、残り392件を`未実施`として登録した。広い範囲をまとめた実機確認だけから個々の項目を一律に`合格`とは扱わない。
...
自動テスト名と試験IDは現状1対1で機械的に関連付けられていない。このため、自動テスト成功は対応領域の証跡として利用するが、関連する全項目を自動的に合格扱いにはしない。
...
| 状態 | 試験ID | 根拠または残作業 |
| --- | --- | --- |
| 証跡あり | `REL-001`、`REL-005`〜`REL-007`、`REL-018`〜`REL-020`、`REL-024`〜`REL-027` | 第3節の自動検査および署名証明書照合 |
| 一部確認 | `REL-002`〜`REL-004`、`REL-009`、`REL-011`〜`REL-017`、`REL-021`、`REL-022` | テストや成果物は存在するが、項目が要求するレビュー・実機条件・外部突合・commit対応の一部が未記録 |
| 未完了 | `REL-008`、`REL-010`、`REL-023` | 全テーマ・最大フォント、全P0/P1集計、署名異常系を項目どおりに確認する |
