...
| `DEV_AUTO` | `DEV` | 239 | UNIT、Repository、DB、計算、通知スケジューラ、再現性のあるinstrumented UI等の自動検査 |
| `RELEASE_OWNER` | `OWNER` | 184 | 手動UI、E2E、実機環境、Console、法的文書および公開判定 |

各試験IDには正式確認の責任者を1件だけ設定する。外部テスターの自由操作で正確な条件と期待結果まで確認できた場合も、`OWNER`が報告内容を照合し、再現条件、版、端末および証跡を記録した後に項目別結果へ反映する。

`DEV_AUTO`の専用テスト239件は[自動試験証跡TSV](automated-test-evidence.tsv)でJUnitテストと1対1に対応する。既存テストが包括的に成功しただけで、未対応の試験IDを合格にしない。
