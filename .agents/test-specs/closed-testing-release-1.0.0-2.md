...
- 状態: Play Console登録・Closed testing公開済み／上書き更新確認済み
- 作成日: 2026-09-07
...
2. テスト専用のカテゴリを作り、色、アイコンおよび並び順を変更する。設定画面でアプリ共通の一日の終了時刻も変更する。
...
- [x] アプリが起動し、初期化や再同意を不必要に要求しない。
- [x] TODO、カテゴリ、色、アイコン、並び順およびアプリ共通の一日の終了時刻が維持される。
- [x] 完了・スキップ履歴と現在の繰り返し状態が維持され、重複発生しない。
- [x] アプリ設定が維持される。
- [x] 通知権限と通知設定が維持され、予定した通知が発火する。
- [x] ウィジェットが読み込みエラーにならず、最新内容を表示して完了操作を反映する。
- [x] 手動バックアップを作成できる。
- [x] クラッシュ、ANRまたはデータ欠損がない。

2026年9月8日にPixel 9a（Android 17 / API 37）で、Google PlayのInternal testing版versionCode `1`からClosed testing版versionCode `2`へアンインストールせず更新し、上記をすべて確認した。更新後の基準バックアップ`B0`を作成し、以後の破壊的試験では上書きしない。
...
- [x] versionCode `1`から`2`へのGoogle Play上書き更新が成功した。
- [x] 更新後のデータ、通知およびウィジェットが維持された。
- [x] 結果を[初回Closed testing実施台帳](closed-testing-log.md)と[初回リリース進行記録](../non-functional-specs/release-specs/initial-release-status.md)へ反映した。
