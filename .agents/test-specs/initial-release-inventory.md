...
2026年9月15日時点では、項目単位の証跡が揃ったP0を238件、P1を129件`合格`、P0/P1の残り38件を`未実施`として登録した。P2を含む全423件では370件が合格、53件が未実施である。今回はTODO編集からのカテゴリ作成、下書きとスクロール位置の一時再生成、タイトルの初期フォーカスとIME操作の3件を追加合格とした。
...
| TODO編集の基本画面・保存境界 | 新規・編集モード、初期値と操作メニュー、カテゴリ順と新規作成、単発・繰り返しの日付UI、方式別一時値、全週開始曜日と部分期間、実変更だけの保存可否、保存結果、未保存確認、再生成時の下書き・位置復元、初期フォーカスを専用unit/Compose UI test 12件で検証 | 2026年9月15日のローカル`testDebugUnitTest`とAPI 34 x86_64`:app:connectedDebugAndroidTest`成功 | `TE-001`、`TE-002`、`TE-004`、`TE-005`、`TE-008`、`TE-010`、`TE-014`、`TE-024`、`TE-026`、`TE-027`、`TE-029`、`TE-D02` |
...
[自動試験証跡TSV](automated-test-evidence.tsv)へ登録した専用テスト298件は試験IDと1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
