...
- TODO登録・編集画面の入力枠とカテゴリ追加・編集画面のカテゴリ名入力枠には、選択中カテゴリの表示色を補助的な所属表現として使用できる。「カテゴリ未設定」では標準の`outline`を使用する。
- カテゴリ色の入力枠でも入力文字とラベルには標準の`onSurface`系Roleを使用し、入力エラー時はカテゴリ色より`error`を優先する。
...
9. カテゴリ色を入力枠へ使用する場合も、ライト・ダークテーマの背景から識別でき、文字色とエラー色の意味を変えない。
