...
現時点では、既知の重大な不具合はなく、versionCode `2`の署名済み公開候補と全自動検査が揃っている。ただし、候補版のGoogle Play配布、全P0/P1項目の項目別結果、必須環境マトリクス、広告、上書き更新、Pre-launch reportおよびClosed testingの証跡が揃っていないため、本番公開判定は`保留`とする。
...
リリース必須のP0/P1は387件である。各項目書の`結果`列は再利用可能な原本として全件`未実施`のまま維持し、リリース候補ごとの実績は[項目別結果TSV](initial-release-results.tsv)へ記録する。2026年9月7日時点では、項目単位の証跡が揃ったP0を111件、P1を14件`合格`、残り262件を`未実施`として登録した。P2を含む全403件では125件が合格、278件が未実施である。広い範囲をまとめた実機確認だけから個々の項目を一律に`合格`とは扱わない。

## 3. 自動検査の証跡

| 対象 | 結果 | 証跡 | 対応する主な項目 |
| --- | --- | --- | --- |
| 単体テスト | 38 test suite、161件、失敗0・エラー0・スキップ0 | 2026-09-04のローカル`testDebugUnitTest`結果、[PR #124 CI run 33859640271](https://github.com/Sakemotti/MATA/actions/runs/33859640271) | `REL-001`、論理日・繰り返し・通知・ViewModel等の一部 |
...
| Instrumented test | API 30 x86_64で54件完了、失敗なし | [PR #126 CI run 33867709111](https://github.com/Sakemotti/MATA/actions/runs/33867709111)。ログに`Starting 54 tests`、`Finished 54 tests`、`BUILD SUCCESSFUL` | `REL-027`、Room Migration・Repository・通知・設定変更・バックアップ・Compose UIの一部 |
...
上表の専用テスト52件は[自動試験証跡TSV](automated-test-evidence.tsv)で1対1に関連付ける。それ以外の自動テスト名と試験IDは現状1対1で機械的に関連付けられていないため、対応領域の証跡として利用しても、関連する全項目を自動的に合格扱いにはしない。
...
| 状態 | 試験ID | 根拠または残作業 |
| --- | --- | --- |
| 証跡あり | `REL-001`、`REL-005`〜`REL-007`、`REL-009`、`REL-011`〜`REL-027` | 第3節の自動検査、公開候補、静的レビュー、外部送信・法的サイト同期照合および署名証明書照合 |
| 一部確認 | `REL-002`〜`REL-004` | テストは存在するが、項目が要求する実機条件・外部突合の一部が未記録 |
